...
export const RefractionDials: React.FC<RefractionDialsProps> = ({ 
  ...
  const [additionalOpticalInfo, setAdditionalOpticalInfo] = useState<string>(() => refraction.additionalOpticalInfo || '');
  ...
  useEffect(() => {
    if (refraction.id !== lastRefractionId.current) {
      ...
      setAdditionalOpticalInfo(refraction.additionalOpticalInfo || '');
    }
  }, [refraction.id]);
  ...
  return (
    <div className="bg-white/90 backdrop-blur-md rounded-3xl border border-slate-200/80 shadow-xs p-5 space-y-4 select-none">
      ...
      <div className="p-4 bg-slate-50/90 border border-slate-200/90 rounded-3xl space-y-3.5 shadow-xs">
              ...
              {[
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    emitUpdate(undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, updated, undefined);
                  }}
        ...
        {/* 🌟 NOVO CAMPO: INFORMAÇÕES ADICIONAIS DO RECEITUÁRIO ÓPTICO (EXCLUSIVO PARA ÓCULOS / DIOPTRIA) */}
        <div className="p-3 bg-blue-50/60 border border-blue-200/90 rounded-2xl space-y-2">
          <div className="flex items-center justify-between">
            <label className="text-[11px] font-black text-blue-950 uppercase tracking-wide flex items-center gap-1.5">
              <Glasses className="w-3.5 h-3.5 text-blue-600" />
              INFORMAÇÕES ADICIONAIS DO RECEITUÁRIO ÓPTICO:
            </label>
            <span className="text-[10px] text-blue-700 font-semibold bg-blue-100/80 px-2 py-0.5 rounded">
              Aparece na Receita de Dioptria
            </span>
          </div>

          <textarea
            rows={2}
            value={additionalOpticalInfo}
            disabled={isReadOnly}
            readOnly={isReadOnly}
            onChange={(e) => {
              if (isReadOnly) return;
              const val = e.target.value;
              setAdditionalOpticalInfo(val);
              emitUpdate(undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, val);
            }}
            ...
            className={`w-full bg-white border border-blue-300 rounded-xl px-3 py-2 text-xs font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-xs ${isReadOnly ? 'cursor-not-allowed bg-slate-100 text-slate-500' : ''}`}
          />

          {/* Chips Rápidos de Informações Adicionais Ópticas */}
          {!isReadOnly && (
            <div className="flex flex-wrap gap-1.5 pt-0.5">
              {[
                'Adaptação progressiva',
                'Uso exclusivo para perto / leitura',
                'Uso contínuo para longe e perto',
                'Filtro fotossensível para luz solar',
                'Armação com aro fechado recomendada',
                'Proteção digital contra luz azul (computador)',
                'Conferir DNP na montagem da armação'
              ].map((info) => (
                <button
                  key={info}
                  type="button"
                  onClick={() => {
                    const updated = additionalOpticalInfo.trim() ? `${additionalOpticalInfo.trim()}. ${info}` : info;
                    setAdditionalOpticalInfo(updated);
                    emitUpdate(undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, updated);
                  }}
